...
if event == "operator_question":
    ...
    if d.get("live"):
        mins = max(1, int(float(d.get("wait_s") or 600) // 60))
        lines[0] = f"⏱ Maro is waiting LIVE on a question ({mins} min) — {goal_short}"
        sent = str(d.get("sent", "") or "").strip()
        if sent:
            lines.append(f"Sent: {sent[:200]}")
    elif dl:
        lines.append(f"Waiting until {dl}")
    unv = d.get("unverified") or []
    if unv:
        lines.append("⚠ Unverified: " + "; ".join(str(u) for u in unv)[:300])
