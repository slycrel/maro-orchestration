...
- **2026-09-16 (Review round budget — Jeremy: "review rounds 2–3 should be the norm, 6–7 rare"; 31 rounds on the mail arc's item 2 was "held wrong"):** per CODE chunk: r1 = the chunk diff (Skeptic + Architect), r2 = the fix diff only, ONE Skeptic, r3 only on a regression the r2 fixes introduced; then STOP and state the residue. Every finding is verified against the code before a fix (30–50% are hallucinated); the orchestrator triages, the adversary does not decide termination; fixes are class-not-instance. DESIGN docs get one pass (two on a REJECT). Applied from the successor's LoopsBench chunks on (item 1: 3 rounds; item 2: 3 rounds, r3 found one real laundering hole).
- **2026-09-17 (The active dev branch is `successor` — Jeremy: "The current active dev branch should be the succession branch… I think you're not targetting that and making changes on the 'old' maro fork"; and the transition doctrine: "it's via contract, implementation details are a side effect, functionality and compatibility at the high level are the target… the new successor should find its own implementation, separate from a line by line port"):** engine work targets `go/` on `successor` (worktree `maro-wt-successor`, landed by `git push origin successor`); main keeps the Python engine and the shared spec, and is merged FORWARD into successor (6a5b5df0 / fb49a976 this day). What main gains and successor lacks is carried across BY CONTRACT (the functionality and its falsifiers), never as a port — the audit `planning/successor-audit-2026-09-17.md` (on successor) is the ledger: what is BY CONSTRUCTION, what is OWED, what is intentionally different. LoopsBench items 1–2 (landed on the Python main 2026-09-16 — the wrong branch, Jeremy's call: "it's on me") were re-implemented Go-side the same day (93467e10, bfccdb8e). Jev is deferred until the successor is "in a more stable spot".
