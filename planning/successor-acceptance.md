# Successor v1 acceptance — the §8a predicate, live (2026-09-04 MT; the journal stamps are UTC 2026-09-05)
...
with the binary at `4b131044`. The review round that followed bound a
lens to its text in the attempt record (`run_attempt.config.lens_text`,
door-checked), and the later binary refuses this journal at the lensed
attempt `1c52a008` ("a lens is bound by name and text together"): by
design, a fold refuses a history the current driver could not have
written. The journal is unchanged; read it with the binary that wrote
it.
...
| A subsequent production run whose request hash or PolicySelection demonstrably changed because of it | Request hash: run `8505fbbd` (goal "What word does the Sanpete pump-word file contain?", before the close; recall 0 included, `stage:candidate`) execute request `s256v1:1d9ed2cf…a21d5`, deliverable "I don't have context for what the Sanpete pump-word file is"; run `97a0cab1` (same goal text, after the close; recall `1 included of 19 [01M1QMDS65EHXMWMARETJGGD5C]`) execute request `s256v1:b198c533…caf5f`, deliverable "The Sanpete pump-word file contains: **juniper**". Same goal thought, different request, and the difference is exactly the lesson's rendering: the before request is 50 bytes (the goal text); the after request is 300 bytes and begins with those 50 bytes, followed by `\n\n## Recalled lessons\n- ` and the lesson text — the fold's NOW rule (`fold.go`, "the request is EXACTLY goal+rendering") re-derives the after request from the goal thought and the recall selection, so the decomposition is checked on every fold, not read off once. PolicySelection: run `abd560a5` (before the ablation closed) `policy 2 of 2 enabled`; run `b30484d6` (after) `policy 01M1QNERVKMRY02CNFDZZQNKSS: 1 of 2 enabled, 1 excluded; mechanisms model_judge=false recall=true`. |
...
| One mechanism removed with absence proof | The `model_judge` seed is `tombstone` (`learn list`); the next production goal, run `b30484d6`, selected `1 of 2` policies with the tombstoned seed in `Excluded` and `Mechanisms[model_judge]=false` in its attempt config — the fold refuses a config that disagrees with the selection. The exclusion entry as the selection `01M1QNERVKMRY02CNFDZZQNKSS` recorded it: `{item: 01M1QKRMT8XV42X4SB8MWCK8GV, revision: 01M1QKRMT8XV42X4SB8MWCK8GW, stage: tombstone, basis: 01M1QNEJQR4QGX4BDEF8HMG9RV, reason: standing}` — the basis is the tombstone transition the evaluator lane committed when experiment `01M1QMWMEN6SMZ3D60NXG9WV98` closed (`runs show` prints each entry since the review round). Step 11b's live run removed `recall` the same way on the height population. |
