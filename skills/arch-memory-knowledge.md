...
**A rewrite must also deal with UNPARSEABLE rows** (2026-08-16). `load_tiered_lessons` silently skips any line it cannot turn into a `TieredLesson`, so rebuilding the file from the parsed list permanently deletes those rows — against the store's own "decay trust, never data" decree. `_rewrite_tiered_lessons` / `_mutate_tiered_lessons` call `_quarantine_unparseable(path)` first, which moves them to a `lessons.jsonl.unparseable` sidecar (append-only, so it is idempotent) and shrinks the live file. Anything hand-rolled must do the same. Two traps found the hard way: carrying the rows back into the live file instead made them immortal (no code path could remove one, and a pre-schema shadow row could resurrect a forgotten lesson), and the numeric fields must be coerced inside the loader's per-row guard — `score`/`confidence`/`sessions_validated` are compared against numbers in code paths with no enclosing try, so one string-typed field wedged every write on the tier while the read path hid the row.

**`TieredLesson.scope`** (§14a slice 3, 2026-08-15): the mint-time `method`/`world` provenance stamp. Write-once, filled from the first real mint that offers one, never flipped. Validate with `knowledge_web.coerce_scope` — type first, then vocabulary, because store rows are untyped JSON and `["world"] in frozenset` raises. Consumed by the census (`camera_readout --portability`) ONLY: it is deliberately **not** a ranking input (decision e2b83703 — ranking uses earned globality, `portability.py`), and `tests/test_lesson_scope.py` tripwires that.
