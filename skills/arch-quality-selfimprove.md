...
The infrastructure is ~90% built. Applied-change verify→learn is closed for both the evolver-suggestion (V2) and graduation (V3) lanes, and the navigator half closed 2026-07-14: **V4** LLM-adjudicates navigator/pipeline divergences at cadence (`adjudicate_navigator_divergences`, append-only `NAVIGATOR_ADJUDICATED`, surfaced in `--agreement`); **V5** crystallizes `pipeline_right` clusters into corrective navigator lessons (`navigator_lessons.jsonl`) injected into `decide()` via the worker-slice seam (A/B flag `navigator.lesson_inject`; readout: `navigator_shadow --agreement` by-lesson-inject table, swarm-review chunk 7). Both gated OFF by default (LLM spend). Remaining gap: inspector↔evolver data sharing. The coordination-discretion readout (`PYTHONPATH=src python3 -m discretion_readout`, chunk 7) tabulates the flag-only gate lanes (second-family/council/cross-ref), METACOGNITIVE_DECISION retry/replan discretion, recall reinjection volume, lesson novelty, and per-day EFFORT — read-only, no flags.
