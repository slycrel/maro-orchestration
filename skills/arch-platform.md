...
The 2026-08-06 census extended that to the legacy vars: **any** workspace pin (`MARO_WORKSPACE` / `OPENCLAW_WORKSPACE` / `WORKSPACE_ROOT`) now means the workspace IS that path — previously a legacy pin routed `memory_dir()`/`projects_root()`/`output_root()` into `<ws>/prototypes/maro-orchestration/` (the shadow-store hijack, BACKLOG census item 5). The one remaining orch-layout data pin is `MARO_ORCH_ROOT` set **alone** (containers / `build-loop.sh` no-arg repo-local mode — data deliberately rides the pinned orch root). `orch_root()` itself is unchanged: it resolves the *code* root (workers/, repo personas/) and still prefers `<ws>/prototypes/maro-orchestration` under any workspace pin for test isolation — a handful of runtime writers (`hooks.py` `.hooks/`, `persona.py` `agents/` manifest, `director.py` projectless `artifacts/director/`, `checkpoint.py` legacy fallback) still anchor data on it; migrating those to workspace roots is the tracked follow-up that would let orch_root become a pure code-root resolver.
