...
**Process-birth identity (`process_identity.py`).** PID-owned ephemeral resources must record both PID and a process-start token: Linux boot ID + `/proc` start ticks, Darwin `libproc` microsecond start time, or pinned UTC/C `ps` on other Unix. Never switch methods after a read failure; ambiguity means retain, not delete. Docker `maro.owner_pid`/`maro.owner_start` labels and trusted scratch-clone sidecars share this helper, so PID reuse is distinguishable without weakening the no-false-deletion boundary.

**Substrate notify hook (notify.py).** How an external substrate (OpenClaw, Hermes, shell) learns a run finished or a human is needed — `docs/SUBSTRATE_INTEGRATION.md` is the contract. `notify.emit(event_type, payload)`: always appends to `memory/events.jsonl` (via observe.write_event) so polling works; additionally runs config `notify.command` with the payload JSON on stdin + `MARO_EVENT_TYPE`/`MARO_HANDLE_ID`/`MARO_STATUS`/`MARO_RUN_DIR` env when the event is in `notify.events` (default `[run_completed, escalation]`). Off by default; bounded by `notify.timeout_seconds` (30); never raises. Emit sites: handle.py finalize (`run_completed`, payload = the run_card — curation feeds notification), navigator blocked-step escalate (`escalation`, point=blocked_step — run-dir exists, finalize also emits run_completed), navigator dispatch-escalate (`escalation`, point=dispatch — run prevented, no run-dir, this is the only signal out), director surface adjudication (`escalation`, point=director_escalation). Since 2026-07-27 (§9.6) escalation payloads additively carry `decision` (single-chasm ask, `escalation_context.decision_line`) and `family_roi` (blocked_step recurrence line keyed on diagnose_loop failure_class, `family_roi_line`) — pure/deterministic, enrichment failure never breaks the emit. `notify_telegram.py` (`maro-notify-telegram`) is the shipped Telegram target: formats card/escalation → plain-text send via telegram_listener resolvers (env → **maro config `telegram.chat_id`/`chat_ids`** → legacy openclaw.json). Do NOT add a server/daemon variant — pull-based drain + in-lifecycle hooks are the invariant.

**Post-goal curation (run_curation.py).** Called by the shared run-lifecycle close hook after terminal metadata is written. `curate_run(handle_id, status)` first uses `build_run_card()` to construct and atomically persist the side-effect-free classification/inventory card, then calls explicit `maintain_run_card()` work (skills-lite promotion and candidate flagging) and atomically persists the enriched card. The phase-aware registry derives `CURATORS` and `MAINTENANCE` from declared provides/requires contracts. Every action records `completed`, `failed`, or `skipped_dependency`; only a producer execution failure/skip blocks consumers (an optional provided key may legitimately be absent). User-visible/prunable: `python3 -m run_curation list|show|curate|prune|repair-audits`. Audit repair uses classification-only refresh and never replays trust-bearing maintenance. Best-effort, never affects request outcome. *Intent:* don't discard paid-for runs — park them for later mining and keep them visible/prunable to the user, while keeping trust-bearing overlay mutation out of pure card construction.
...
**Autonomy switch:** `heartbeat_loop(..., autonomy=False)` is health-only by default. Scheduler drain, task-store drain, mission drain, backlog drain, evolver, inspector, and eval work only run when autonomy is explicitly enabled via CLI/config. Before an autonomy-mode evolver pass, heartbeat also attempts up to three pending audit repairs. This shares the existing cadence rather than adding a daemon; verdict replay is local and the cheap adapter is constructed lazily only for deferred learning.
...
## Workspace Routing (RESOLVED 2026-07-03; legacy pins unified 2026-08-06)
...
The 2026-08-06 census extended that to the legacy vars: **any** workspace pin (`MARO_WORKSPACE` / `OPENCLAW_WORKSPACE` / `WORKSPACE_ROOT`) now means the workspace IS that path — previously a legacy pin routed `memory_dir()`/`projects_root()`/`output_root()` into `<ws>/prototypes/maro-orchestration/` (the shadow-store hijack, BACKLOG census item 5). The one remaining orch-layout data pin is `MARO_ORCH_ROOT` set **alone** (containers / `build-loop.sh` no-arg repo-local mode — data deliberately rides the pinned orch root). `orch_root()` itself resolves the *code* root (workers/, repo personas/) and still prefers `<ws>/prototypes/maro-orchestration` under any workspace pin for test isolation. The same-day 5b writers migration moved the last runtime-data anchoring off it: hooks registry → `<ws>/.hooks/` (legacy read-fallback), persona `agents/` manifest and projectless director logs → `output_root()`, non-run-dir checkpoints → `<ws>/checkpoints/` (read/consume fallback to the old `orch_root()/checkpoints`), and git-cwd/BACKLOG.md consumers (captains_log, convo_miner) → the new **`repo_root()`** (the source checkout, pin-independent). Same-day adversarial-review fix added **`data_root()`** — the anchor for workspace data that isn't memory/projects/output (checkpoints, hooks registry): `orch_root()` when `MARO_ORCH_ROOT` is the only pin, else `config.workspace_root()` — so repo-local/container mode keeps its data (and, critically, its *executable hook registry*) local instead of reaching into the production workspace. Old-location fallbacks are deliberately env-relative: a pinned env does not see files written unpinned. Residual (BACKLOG 5b): drop the prototype shape from `orch_root()` entirely — churny test sweep, deferred; fold in `orch.py:413`'s validation-summary fallback writer.
