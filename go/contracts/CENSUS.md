...
| `terminal_observed` | production | `terminal_observed/1` | invoke.Shell | invoke.State fold; Reconcile | how the stream ended, before any receipt exists | forever |
...
| `tool_effect` | production | `tool_effect/1` | invoke.Shell (as the backend stream reports) | judges (provenance/fabrication); Reconcile; the receipts view | what the backend actually did; per-effect keys for reconciliation | forever |
