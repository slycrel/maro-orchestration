...
what no B-entry can: the whole learn ledger (revisions, transitions,
exposures, selections) and the experiment evidence — exactly as the
source journal framed them, plus the cited lesson_text thoughts by hash.
It is Go-only causal history. The importer vouches for a pack by
FOLDING its production records under the learn ledger's own rules
(`learn.FoldRecords`): what the source's current revisions are and what
they had reached is the fold's answer, never a record's claim; a
history the fold refuses enters nothing, and neither does a header
that disagrees with the body, an uncited or non-lesson thought, or a
body that is not its hash. An import never adopts a stage: every
current lesson of the source (not tombstoned, not quarantined) enters
HERE at candidate with `import` provenance — `ref` the source revision,
`origin` `pack:<source revision>` — in ONE journal command; a source
lesson already here (by origin) is reported, and a source lesson revised
since it entered becomes a new revision of the same local item. Only
an experiment arm this workspace opens on it can apply an import before
it is staged: that is the door a candidate earns standing through, not
a bypass. `pack import-python <dir>` reads a Python workspace's three
B7 tiers the same way (highest tier wins per `lesson_id`; what the
Python readers would not inject is skipped by name — a row the flat
reader cannot build, prompt-minted, truthy `contested`, provisional
TIERED rows; origin `python:<label>:<lesson_id>:<text digest>`, label
defaulting to the store's absolute path). No round trip is claimed.
Thought bodies are stored (content-addressed) after everything is
vouched for and before the one command: a crash between the two leaves
bodies no record cites, which is cache, not state.
