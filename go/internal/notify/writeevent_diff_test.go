...
sys.stdout.write(open(observe._events_path(), encoding="utf-8").read())
`
...
func TestWriteEventFieldsMatchCPython(t *testing.T) {
	...
	cases := []struct {
		...
		{"the zero value", "loop_start", EventFields{}},
		{"goal at its bound", "step_done", EventFields{Goal: strings.Repeat("g", 80)}},
		{"goal one past it", "step_done", EventFields{Goal: strings.Repeat("g", 81)}},
		{"goal far past it, silently", "step_done", EventFields{Goal: long}},
		{"step at its bound", "step_done", EventFields{Step: strings.Repeat("s", 120)}},
		{"step one past it", "step_done", EventFields{Step: strings.Repeat("s", 121)}},
		// detail is the ONE announcing cut in the row. If a port clips all
		// three the same way, this case is the only one that says so.
		{"detail at its bound", "step_done", EventFields{Detail: strings.Repeat("d", 200)}},
		{"detail one past it", "step_done", EventFields{Detail: strings.Repeat("d", 201)}},
		{"detail far past it, loudly", "step_done", EventFields{Detail: long}},
		{"multi-byte at every bound", "step_done", EventFields{
			Goal: wide, Step: wide, Detail: wide}},
		{"negative and huge counters", "step_done", EventFields{
			StepIdx: -1, TokensIn: -5, TokensOut: 1 << 40, ElapsedMs: -12}},
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			want := strings.TrimRight(
				runPy(t, pyWriteEventSrc, pyWS, string(pyKwargs(t, c.event, c.fields))), "\n")

			goWS := t.TempDir()
			WriteEvent(goWS, c.event, c.fields)
...
// Every case above states project and loop_id explicitly, because in Go
// they must be — their zero is nil, which spells null, and Python's
// keyword default is "". That gap is the price of carrying a raw value at
// all, and this is the test that says so: a truly bare write_event(type)
// on the CPython side against the Go call that reproduces it.
func TestABareCallMatchesCPythonsKeywordDefaults(t *testing.T) {
	want := strings.TrimRight(
		runPy(t, pyWriteEventSrc, t.TempDir(), `{"event_type": "loop_start"}`), "\n")

	ws := t.TempDir()
	WriteEvent(ws, "loop_start", EventFields{Project: "", LoopID: ""})
	...
	fws := t.TempDir()
	WriteEvent(fws, "step_done", EventFields{Project: 1.0, LoopID: ""})
	...
	// And the trap this documents: the Go ZERO VALUE is not that row.
	zero := t.TempDir()
	WriteEvent(zero, "loop_start", EventFields{})
	rawZero, err := os.ReadFile(filepath.Join(zero, "memory", "events.jsonl"))
	if err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(string(rawZero), `"project": null`) {
		t.Errorf("the zero value no longer spells null; the doc comment on "+
			"EventFields and the two explicit callers need revisiting:\n%s", rawZero)
	}
}
