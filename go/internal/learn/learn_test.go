...
func TestStandingIsPerRevision(t *testing.T) {
	...
	if sel := Recall(led, q); len(sel.Included) != 0 || sel.ExcludedCounts["stage:candidate"] != 1 || sel.ExcludedTop[0].Revision != r2.ID {
...
// is bounded to TopK with counts carrying the rest.
func TestRecallIsDeterministicAndBounded(t *testing.T) {
	...
	items := []*LearnedRevision{
		...
		rev(LearnedID(record.NewID()), "", Lesson, ScopeGoal(goal), "", lessonRef(st, "goal lesson")),
		rev(LearnedID(record.NewID()), "", Lesson, ScopeGoal(record.NewID()), "", lessonRef(st, "other goal")),
	...
	for k := 0; k < 7; k++ { // more excluded items than TopK
	...
	if len(sel.Included) != 3 || sel.Considered != 13 {
		t.Fatalf("%+v", sel)
	}
	...
	want := map[string]int{"scope": 1, "family": 1, "kind:policy": 1, "stage:candidate": 7}
	for k, v := range want {
		if sel.ExcludedCounts[k] != v {
			t.Fatalf("exclusions: %v", sel.ExcludedCounts)
		}
	}
	if len(sel.ExcludedTop) != TopK || sel.ProjectedBytes != int64(len(heading))+int64(len("ws lesson")+len("goal lesson")+len("right family"))+3*int64(len(bullet)+1) {
		t.Fatalf("bound/size: %d %d", len(sel.ExcludedTop), sel.ProjectedBytes)
	}
...
func TestFoldRefusesIllegalLearnedHistories(t *testing.T) {
	...
	fold_ := []struct {
		...
		{"evidence not an earlier record", []record.Record{func() record.Record {
			x := tr(item, r1.ID, Candidate, Provisional)
			x.Evidence = record.NewID()
			return x
		}()}, "not an earlier record"},
