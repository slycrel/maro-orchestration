...
func samples() map[record.Kind]any {
	...
	return map[record.Kind]any{
		...
		experiment.KindMeasurement:  &experiment.EffectMeasurement{Header: withSubject(withSchema(h, "effect_measurement/1"), record.Ref{Kind: "experiment", ID: string(inv)}), Experiment: inv, Attestation: inv, Hypothesis: learn.ItemRev{Item: learn.LearnedID(inv), Revision: inv}, Relation: experiment.ApplyItem, Assigned: 2, Analyzed: 2, Exposed: 2, Discordant: 1, TreatmentN: 1, ControlN: 1, DeltaITT: 1, DeltaPP: 1, Verdict: experiment.TreatmentHelpful, ItemEffect: learn.ItemHelpful},
