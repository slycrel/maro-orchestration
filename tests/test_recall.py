...
class TestAncestryFallbackRetired:
    """The project-ancestry (goal-slug) thread fallback is gone (2026-09-05):
    the landscape decides a goal's relation before it runs and stamps the
    decided parent on the origin. With no origin and no current run, recall
    knows no thread — even when the project's ancestry.json says otherwise."""

    def test_no_origin_means_no_thread_even_with_ancestry(self, monkeypatch, tmp_path):
        _setup(monkeypatch, tmp_path)
        from orch_items import project_dir
        pd = project_dir("child-proj")
        pd.mkdir(parents=True, exist_ok=True)
        (pd / "ancestry.json").write_text(json.dumps({
            "parent_id": "parent-proj",
            "ancestry": [{"id": "top-mission", "title": "Ship the orchestrator"},
                         {"id": "parent-proj", "title": "Build the memory layer"}],
        }), encoding="utf-8")
        for slc in ("dispatch", "loop"):
            r = recall("implement recall cache", slice=slc, project="child-proj")
            assert r.thread is None
            assert r.sources["thread_source"] == ""
        import recall as recall_mod
        assert not hasattr(recall_mod, "_thread_from_project_ancestry")

    def test_origin_walk_still_names_the_thread(self, monkeypatch, tmp_path):
        _setup(monkeypatch, tmp_path)
