...
Last updated: 2026-07-12 (Opus parallel session, alongside Sonnet on -5 #4
routing: **-5 #6 container-executor C1 SHIPPED** — image + auth + doctor +
the `src/container_exec.py` seam C2 extends; disjoint file set from routing,
no conflict; see -5 #6. C2 = the wrap is next, Opus-tier.). Previous:
2026-07-12 (Sonnet execution session: -5 #4 Routing Part A SHIPPED —
needs_live_data classifier signal + capability override close the Manti
canonical-case routing gap, live-verified at 0.95 confidence with no
...
-5. **Post-handoff execution queue (2026-07-12)** — *the ordered chunk list
   ...
   9. **Verify→learn arc V1–V5** (`docs/VERIFY_LEARN_ARC.md`) — post-1.0 by
      decree; V0=B3 is the only pre-1.0 piece.
   *Still Jeremy-gated (not in the queue): PyPI **1.0.0** publish + tag (name
   RESERVED 2026-07-12 — `maro-orchestration` 0.8.0 live via OIDC trusted
   publishing), evolver production hours (rides live batches), #24
   model-route session (in progress), container flip (C4).*
