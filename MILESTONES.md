...
- [ ] **5. Write the navigator prompt** — last. Hand-write against the schema, then replay against 3–5 representative threads from `~/.poe/workspace/runs/` in shadow mode and see what it decides (the 2026-05-18 plan). Includes the shadow-harness callsite choice and the per-thread goal-brain stand-in (scope/resolved-intent per NAVIGATOR_SCHEMA.md open ends).
