...
def _handle_budget_ceiling(
    ...
    if remaining_steps:
        try:
            ...
            _parent_handle = ctx.handle_id
            try:
                from runs import current_handle_id as _cur_hid
                _parent_handle = _parent_handle or _cur_hid() or ""
            except Exception:
                _parent_handle = _parent_handle or ""
            _origin = Origin(
                parent_loop_id=ctx.loop_id,
                parent_handle_id=_parent_handle,
                parent_goal=ctx.goal[:200],
            )
            if ctx.measurement_class:
                _origin["measurement_class"] = ctx.measurement_class
