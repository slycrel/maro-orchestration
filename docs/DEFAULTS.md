...
| `navigator.act_dispatch` / `navigator.act_blocked_step` | `False` | Master switches for navigator decisions *executing*. **Deliberately left OFF on this box — flipping is Jeremy's call** (escalate lane is READY; organic close evidence wanted next). |
