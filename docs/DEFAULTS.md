...
| `keep_artifacts` | `False` | Per-step artifacts are cleaned unless a run opts in — but cleanup is **deferred** since 2026-07-10 (BACKLOG #18: the goal verdict is judged after the loop returns, and immediate deletion destroyed the evidence needed to audit a semantic miss). Each finalize sweeps *other* loops' `loop-*-step-*.md` past a 24h grace window (`loop_finalize.cleanup_step_artifacts`), never the just-finished loop's. Flip ON to retain everything for debugging; watch disk (this box: 156G shared with everything). |
