...
| `validate.local_max_latency_ms` | `15000` | Latency breaker for the free local validation tier (2026-07-10 envelope arc): the first local verdict slower than this cap trips a per-process breaker and the ladder goes straight to paid for the rest of the process. Production ROI on the Mac Mini: local averaged 35.8s/call vs 6.5s paid, saving ~$0.64 *lifetime* — one 11-step run spent 454s (27% of wall clock) on local verdicts that all passed. Each new process re-probes once, so a faster box/model self-heals. `0` disables the breaker (local tier always runs). |
