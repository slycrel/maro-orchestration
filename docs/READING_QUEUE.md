...
| 2026-08-07 | [Δ-gate validation + fix-review record](https://github.com/slycrel/maro-orchestration/blob/main/docs/history/2026-08-06-delta-gate-validation.md) | Overnight per your "fix those, then the gate": all 6 review-fix chunks landed, then Δ-gate slice 1 SHIPPED — instrument validated against pre-registered predictions (effective +0.59 / inert −0.06 / rule **−0.15**, separation criterion passes) and the effect route to LONG is live behind it (Δ≥0.30, killswitch on, CLI-driven measurement only). **Your keep/adjust call rides the routes census — now WRITTEN** (2026-08-07 ~12:48 on subprocess haiku, readout appended to the doc): top 3 corpus lessons by score ALL measure mildly negative on decisions (Δ −0.08..−0.10, tight spreads, 0 errors) — nothing promotes, and score rank is exactly what Δ contradicts. The rule specimen's negative Δ from validation is the same direction. Worth 5 minutes now. |
