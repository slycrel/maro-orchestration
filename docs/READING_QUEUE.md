...
|---|---|---|
| 2026-08-05 | [BACKLOG § LT-4 batch results](https://github.com/slycrel/maro-orchestration/blob/main/BACKLOG.md) | Your directed cold/warm batch, complete same day: **12/12 PASS, warm = −49% on byte-identical arms** — artifacts/continuity are the carriers (LT-1's lessons-carry-nothing inverted), and B1w proved the skill store amortizes (cold-minted skills injected, manifest-verified). Per-arm evidence: `~/.maro/workspace/output/lt4-logs/scorecard.md`. **One decision:** fix direction for the skill dead-drop bug (workers write skills to `projects/<slug>/skills/`, which nothing reads — reproduced 2/2; both stranded ladders hand-rescued to the overlay already). My recommendation in the entry: promotion-side ingest, which keeps one vetted write-path with tier/breaker guardrails. Also note the claims-vs-events finding family (3 instances; reuse propagates false provenance) — follow-up (c) proposes mint-time grounding, no decision needed yet. |
...
| 2026-08-03 | [BACKLOG § LeAct — 2026-08-03 amendment](https://github.com/slycrel/maro-orchestration/blob/main/BACKLOG.md) | Your Aug-3 Opus contrast folded in, all code claims verified. **The item's own revisit trigger fired** — verdict_flow now 67 judged rows, arrivals two weeks running (W31 18, W32 9) vs the 4 that made us defer; and the prerequisite census found the last listed blocker stale (replay capture live since 2026-07-29 — 116 run dirs, including the LT-1 arms, so the held-out set and the fixture source are the same runs). Remaining work is one wire, not an arc. **Two decisions:** (1) priority against SP / world-facts — this is a knowledge-layer arc opening while both are in flight; (2) a go on the seed-reader style-exemplar A/B (memory.py:267 still says "emulate this style and specificity"; LeAct Fig 3b says that's backwards) — hours not an arc, and it should run BEFORE the Δ gate or Δ measures a corpus we already suspect is contaminated. |
