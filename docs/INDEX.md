...
| Why do imported workspaces still hold the source machine's paths? | `PATH_PORTABILITY_DESIGN.md` (living; SHIPPED 2026-08-18 — export-side `%%MARO_*%%` placeholders, owned-vs-observed by construction, byte-exact round trip) |
...
| World-fact capture / injection / landing (plan-native non-action findings)? | `WORLD_FACTS_DESIGN.md` (living; all 3 slices shipped, §7 re-called 2026-08-11) |
| Compound thinking — self-surveying map, taste/camera/player, work-depth (§14)? | `COMPOUND_THINKING_DESIGN.md` (dormant-design per its own frontmatter; newest section §14i 2026-08-15 — §14a scope slices 1–3 shipped, record in §14g) |
| Mint-time grounding — evidence receipts on lesson/skill mints (claims-vs-events family)? | `MINT_GROUNDING_DESIGN.md` (living; shape decided 2026-08-06 — annotation not judge, fail-closed at republish only; slice 1 shipped) |
| Δ-gate (effect-based lesson promotion) — what to build and in what order? | `DELTA_GATE_BUILD_BRIEF.md` (living; priority decided 2026-08-06, prep for the next session's build) |
| Portable/shareable learning — migration + learning packs (1.0 item (g))? | `PORTABLE_LEARNING_DESIGN.md` (dormant-design; §8 RATIFIED 2026-07-12 — all chunks 1-4 shipped, minimum 1.0 slice complete. EXCEPTION: §3's transport-contract prose is LIVE and load-bearing — it is the only record of what crosses the pack border, including the §14a scope stamp) |
...
| Where do credentials live, how do they reach a container / the Go engine, how does a run hand one back? | `SECRETS_DESIGN.md` (living; sops + age store at `~/.maro/secrets/`, inject policy, presence index, drop file; shipped both engines 2026-09-06) |
| How does a run ask the operator something, and how does the answer get back by handle? (the rare-exception lane) | `OPERATOR_ASK_DESIGN.md` (living; `$MARO_ASK` file contract, typed pause / `needs answer` terminal, `maro answer` / `maro-go answer`, Hermes gate `answer` verb, time box, `asks` ledger; shipped both engines 2026-09-06) |
| How does a run get software it lacks (a browser, a package) without root — and who decides when policy says no? | `ENV_REQUEST_DESIGN.md` (living; `$MARO_ENV_REQUEST` file contract, per-project executor image layers built from a reviewable Dockerfile, policy + grants, `escalation` to the ORCHESTRATOR (Hermes/Poe) with `maro answer <handle> allow\|deny`; decree ea9e311f 2026-09-07) |
