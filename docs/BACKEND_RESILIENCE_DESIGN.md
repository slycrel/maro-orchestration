...
| Checkpoints (checkpoint.py; loop_post_step.py:220-226) | Per-step `ckpt_<loop_id>.json` with full plan + completed[] outcomes; resume wired via `resume_from_loop_id` (agent_loop.py:137 → loop_planning.py:125-160); deleted on done (loop_finalize.py:251) | Three fatal gaps: (1) ~~written to an env-dependent path (`orch_root()/checkpoints`, checkpoint.py:45-53) — 52 stale files sit in the repo checkout, **0** live ones in the workspace~~ **CLOSED 2026-08-06:** run-dir `build/checkpoint.json` is primary; non-run-dir fallback now `<workspace>/checkpoints` via `orch_items.data_root()`, old orch_root location read-fallback; (2) resume is opt-in and no caller outside auto-recovery ever passes `resume_from_loop_id`; (3) the in-flight step is invisible — a reader can't distinguish "step 7 not started" from "step 7 crashed mid-write". |
...
| `OUTPUT_CAP_EXCEEDED` | A capped `no_tools` utility call overran its own `CLAUDE_CODE_MAX_OUTPUT_TOKENS` (the 2026-07-16 hard-error cap). Request-shaped, not backend death: never retry, never failover — propagate so the call site's parse-fallback runs. Added 2026-07-17 after azure-finch failed such a call over to a credit-dead OpenRouter key (402 alert spam to Telegram). Same day, the env cap gained a floor (`llm._OUTPUT_CAP_FLOOR`, 1500): the CLI cap counts thinking tokens, so contract-sized caps (128/300) hard-errored before the model could reason (zesty-ash lost its scope to a 300-cap call). This class now fires only for genuine ≥floor runaways. | Added post-design; azure-finch + zesty-ash specimens. |
...
(runs.py:350-371). Migration (SHIPPED 2026-08-06 for the path move —
non-run-dir checkpoints now workspace-routed): read old
`orch_root()/checkpoints` path as
