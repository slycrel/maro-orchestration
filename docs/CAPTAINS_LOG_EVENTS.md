...
| `LESSON_QUARANTINED` | knowledge_web.py (`set_tiered_lesson_minted_from`) | tier, minted_from, reason | A stored lesson's `minted_from` provenance was (re)stamped — the quarantine/unquarantine maintenance verb for the tiered store. Prompt-derived lessons are quarantined from cross-goal injection surfaces; the row stays on disk either way. |
