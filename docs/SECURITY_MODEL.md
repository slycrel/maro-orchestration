...
| **Bounded worker writes (the fence)** | Executor subprocesses have their cwd bound inside the run's workspace so relative writes land in-fence (`src/llm.py`, binding in `_run_subprocess_safe`). Mandatory loop-entry setup—stale-policy reset, fence-directory creation, cwd binding, and writable-root policy binding—refuses the loop before decomposition if it fails; optional root discovery degrades to cwd-only. Out-of-fence access is detected from the real tool transcript (`artifact_check.detect_out_of_fence_access`); an out-of-fence **write** demotes the step done→blocked, default ON since 2026-07-09. /tmp and goal-declared paths widen the fence. See `docs/BOUNDED_WORKSPACE.md`. This is drift detection, not containment — a hostile host worker can still write anywhere; the fence makes it visible and fails the step. |
...
| **Hosted validation egress** | The Groq/Gemini hosted-free validation rung is OFF by default even when credentials exist (`validate.hosted_free.enabled: false`). Enabling it explicitly permits step/validation payloads to leave the machine for the configured providers; request duration is bounded by `validate.hosted_free.max_latency_ms`. Paid model calls retain their existing provider boundary. |
