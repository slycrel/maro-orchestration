...
- [ ] **LT-1 — the batch (8 goals, cold+warm pairs).** Mostly `target`
  ...
  | 5 | 🔄 **COLD ARM DISPATCHED 2026-08-02** (`9d88acf2-fierce-shore`, Systemantics/John Gall 1975, reframed: every claim source-listed, "insufficient information found" per-aspect rather than gap-filling). **Prediction registered: $5–15 / 15–30min**; pass = catalog/archive-grounded description with a real source list; the corpus-1.6 failure shape to watch for = describing from memory with a decorative source list | retrieval-before-describe, per-aspect honesty | corpus 1.6/1.10, Tier 1 `target` |
