...
- [ ] **Model bake-off.** Compare candidate local validators (VibeThinker-3B 8bit vs
  ...
  (≥16 GB RAM; 4-bit for 8 GB) before standardizing on one. **Current evidence
  is useful but not the formal sweep:** M1 Max measurements selected
  VibeThinker-3B-4bit (14/14, 8.2s) and rejected 1.5B-4bit (4/8, 12.5s),
  3B-8bit (6/6, 16.5s), and Qwopus-27B (3/4, 21.4s), but those denominators
  are different subsets. Six path/constraint cases behind the 14/14 headline
  are not committed fixtures. Remaining proof: commit the full corpus, replay
  every candidate against exactly it, and include the production Linux/Ollama
  option (plus Qwen2.5-Coder exact-protocol, not only its n=42 shadow data).
