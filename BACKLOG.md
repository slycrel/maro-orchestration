...
**Status 2026-09-18 — the session is dead again, and the warning did not fire.** A live container call with the shared volume comes back `Failed to authenticate: OAuth session expired and could not be refreshed` (401), while `memory/container_auth_liveness.json` still records `ok: true` with `refresh_expires_at` = 2026-10-11 and a `checked_at` of 2026-09-13 02:21. So the monthly-expiry model is measuring the wrong thing: the refresh TOKEN's nominal expiry is not what governs, the refresh itself is failing five days later, and because the probe is cadence-gated on that record nothing re-checked. Consequences today: every container-required run on this box fails at its first tool-bearing call (`executor.container: require` has been live since 09-13), and the same volume is what the Go engine's new container lane uses (`--executor require`, landed 2026-09-18, `planning/successor-audit-2026-09-17.md` §4.5). Two owed, beyond the re-seed itself: the liveness record must be invalidated by a FAILED call, not only by its own cadence (a 401 from a real call is the strongest observation available and today it updates nothing), and the warning should key on observed refresh failure rather than the token's advertised expiry. Found while measuring the box for the successor re-orient plan, not by a probe — which is the point.
